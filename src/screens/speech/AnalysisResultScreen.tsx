...
const C = {
  ...
  textHint:'rgba(241,245,249,0.30)',
  green:   '#10B981',
  accent:  '#A78BFA',
  gold:    '#F59E0B',
  danger:  '#F43F5E',
...
export default function AnalysisResultScreen({ navigation, route }:any) {
  ...
  const generateFeedback = () => {
    ...
    if (fillerCount === 0) {
      tips.push({ type:'pos', icon:'checkmark-circle', color:'#10B981',
      ...
      tips.push({ type:'warn', icon:'warning', color:'#F59E0B',
      ...
      tips.push({ type:'neg', icon:'close-circle', color:'#F43F5E',
        title:`${fillerCount} Filler Words — Needs Work`,
        text:`Filler words appeared frequently. Top offenders: ${top3}. Practice the pause technique: when you feel an "um" coming, take a breath instead.` });
    }

    if (wpm >= 110 && wpm <= 150) {
      tips.push({ type:'pos', icon:'speedometer', color:'#8B5CF6',
      ...
      tips.push({ type:'warn', icon:'speedometer', color:'#F59E0B',
        title:`Speaking Too Fast: ${wpm} WPM`,
        text:`You spoke at ${wpm} WPM — slightly above the ideal range. Try pausing for 2 seconds after key points.` });
    } else if (wpm > 0 && wpm < 110) {
      tips.push({ type:'warn', icon:'speedometer', color:'#F59E0B',
        title:`Speaking Too Slow: ${wpm} WPM`,
        text:`At ${wpm} WPM your pace may lose listeners. Aim for 120-140 WPM. Practice reading aloud daily.` });
    }

    if (details.clarity >= 85) {
      tips.push({ type:'pos', icon:'mic', color:'#10B981',
      ...
      tips.push({ type:'warn', icon:'mic', color:'#F59E0B',
        title:'Improve Articulation',
        text:'Some words were unclear. Practice tongue twisters daily to strengthen articulation muscles. Open your mouth wider when speaking.' });
    }

    if (details.confidence >= 80) {
      tips.push({ type:'pos', icon:'trending-up', color:'#8B5CF6',
        title:'Strong Confidence',
        text:'Your tone projected confidence. Maintaining steady eye contact and upright posture reinforces this verbal confidence.' });
    } else {
      tips.push({ type:'warn', icon:'trending-up', color:'#F59E0B',
        title:'Build Confidence',
        text:'Your delivery showed some hesitation. Try the "power pose" for 2 minutes before speaking — it measurably boosts vocal confidence.' });
    }
    ...
    tips.push({ type:'tip', icon:'bulb', color:'#A78BFA',
      title:'Pro Tip',
      ...
      >
        ...
        <View style={s.actionsRow}>
          ...
          <TouchableOpacity style={s.primBtn} onPress={()=>navigation.navigate('SpeechHome')} activeOpacity={0.85}>
            <LinearGradient colors={['#8B5CF6','#4338CA']} start={{x:0,y:0}} end={{x:1,y:0}} style={s.primBtnGrad}>
...
const s = StyleSheet.create({
  ...
  fillerCard:    {backgroundColor:'rgba(245,158,11,0.10)',borderRadius:14,padding:16,marginBottom:16,borderWidth:1,borderColor:'rgba(245,158,11,0.25)'},
  ...
  fillerChip:    {flexDirection:'row',alignItems:'center',backgroundColor:'rgba(255,255,255,0.06)',borderRadius:20,paddingLeft:10,paddingRight:4,paddingVertical:4,gap:6,borderWidth:1,borderColor:'rgba(245,158,11,0.30)'},
  fillerWord:    {fontSize:13,fontWeight:'600',color:'#F59E0B'},
  fillerBadge:   {backgroundColor:'#F59E0B',borderRadius:16,paddingHorizontal:8,paddingVertical:2},
  ...
  metricCard:    {width:(W-50)/2,backgroundColor:C.card,borderRadius:14,padding:14,borderWidth:1,borderColor:C.border,gap:6},
  ...
  planCard:      {backgroundColor:'rgba(139,92,246,0.10)',borderRadius:14,padding:16,marginBottom:16,borderWidth:1,borderColor:'rgba(139,92,246,0.20)'},
  planTitle:     {fontSize:15,fontWeight:'700',color:'#A78BFA',marginBottom:12},
  planRow:       {flexDirection:'row',alignItems:'flex-start',gap:10,marginBottom:10},
  planDayBadge:  {backgroundColor:'rgba(139,92,246,0.30)',borderRadius:8,paddingHorizontal:8,paddingVertical:4,minWidth:60,alignItems:'center'},
  planDay:       {fontSize:10,fontWeight:'700',color:'#A78BFA'},
  planTask:      {flex:1,fontSize:12,color:C.textSec,lineHeight:18},
  actionsRow:    {flexDirection:'row',gap:12},
  secBtn:        {flex:1,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,backgroundColor:C.card,borderRadius:12,borderWidth:1,borderColor:C.border,paddingVertical:14},
