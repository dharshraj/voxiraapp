...
function ResultRow({ item, onPress }: { item:SearchResult; onPress:()=>void }) {
  ...
  const rs = StyleSheet.create({
    card:  { flexDirection:'row', alignItems:'center', gap:12, backgroundColor:C.surface, borderRadius:16, padding:14, marginBottom:8, borderWidth:1, borderColor:C.border },
  ...
  return (
    <TouchableOpacity style={rs.card} onPress={onPress} activeOpacity={0.80}>
      ...
      {item.screen && <Ionicons name="chevron-forward" size={16} color={C.textMuted} />}
    </TouchableOpacity>
  );
}
...
export default function SearchScreen({ navigation }: any) {
  ...
  const s = StyleSheet.create({
    ...
    backBtn:   { width:42, height:42, borderRadius:13, backgroundColor:C.surface, borderWidth:1, borderColor:C.border, alignItems:'center', justifyContent:'center' },
    searchBar: { flex:1, flexDirection:'row', alignItems:'center', backgroundColor:C.surface, borderRadius:16, paddingHorizontal:14, height:48, borderWidth:1.5, borderColor:C.border },
    ...
    trendChip: { flexDirection:'row', alignItems:'center', gap:6, paddingHorizontal:12, paddingVertical:8, borderRadius:12, borderWidth:1, borderColor:C.border, backgroundColor:C.surface },
    trendTxt:  { fontSize:12, color:C.textMuted },
      ...
      <Animated.View style={[{flex:1}, { opacity:fadeAnim }]}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.catRow}>
          {CATEGORIES.map(cat => (
            <TouchableOpacity
              key={cat.key}
              style={[s.catChip, catFilter===cat.key && { backgroundColor:`${cat.color}22`, borderColor:`${cat.color}55` }]}
              onPress={() => setCat(cat.key)}
            >
              <Ionicons name={cat.icon as any} size={13} color={catFilter===cat.key ? cat.color : C.textMuted} />
