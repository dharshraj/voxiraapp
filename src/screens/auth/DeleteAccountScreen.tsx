...
export default function DeleteAccountScreen({ navigation }: any) {
  ...
  const ALTERNATIVES = [
    { icon: 'download-outline',           label: 'Export your data first',     desc: 'Download a copy before deleting',  color: C.primary, action: 'export'  },
    { icon: 'pause-circle-outline',       label: 'Pause your account',         desc: 'Take a break, come back anytime',  color: C.warning, action: 'pause'   },
    { icon: 'notifications-off-outline',  label: 'Turn off notifications',     desc: 'Stop reminders without deleting',  color: C.success, action: 'notifs'  },
  ];
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const { error: signInErr } = await supabase.auth.signInWithPassword({ email: user.email, password: password.trim() });
  ...
  const s = StyleSheet.create({
    ...
    header:        { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: Platform.OS === 'ios' ? 56 : 36, paddingBottom: 16, backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.border },
    ...
    stepRow:       { flexDirection: 'row', paddingHorizontal: 40, alignItems: 'center', justifyContent: 'center', paddingVertical: 12, backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.border },
    stepItem:      { flexDirection: 'row', alignItems: 'center' },
    stepCircle:    { width: 28, height: 28, borderRadius: 14, borderWidth: 2, borderColor: C.error + '60', alignItems: 'center', justifyContent: 'center', backgroundColor: C.error + '14' },
    stepCircleDone:{ backgroundColor: C.error, borderColor: C.error },
    stepNum:       { fontSize: 12, fontWeight: '700', color: C.textMuted },
    stepNumDone:   { color: '#fff' },
    stepLine:      { width: 50, height: 2, backgroundColor: C.error + '30', marginHorizontal: 4 },
    stepLineDone:  { backgroundColor: C.error },
    scroll:        { paddingHorizontal: 20, paddingTop: 20 },
    warningIcon:   { width: 88, height: 88, borderRadius: 26, backgroundColor: C.error + '20', borderWidth: 1, borderColor: C.error + '40', alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginBottom: 20 },
    stepTitle:     { fontSize: 22, fontWeight: '800', color: C.text, textAlign: 'center', marginBottom: 10, letterSpacing: -0.5 },
    stepDesc:      { fontSize: 14, color: C.textSec, textAlign: 'center', lineHeight: 22, marginBottom: 24 },
    listTitle:     { fontSize: 13, fontWeight: '700', color: C.text, marginBottom: 10 },
    deleteList:    { backgroundColor: C.surface, borderRadius: 18, overflow: 'hidden', borderWidth: 1, borderColor: C.error + '30', marginBottom: 24 },
    deleteRow:     { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, borderBottomWidth: 1, borderBottomColor: C.border },
    deleteIconWrap:{ width: 34, height: 34, borderRadius: 10, backgroundColor: C.error + '20', alignItems: 'center', justifyContent: 'center' },
    deleteItemTxt: { fontSize: 13, color: C.textSec, flex: 1 },
    continueBtn:   { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderWidth: 1, borderColor: C.error + '60', borderRadius: 16, paddingVertical: 14, marginBottom: 12, backgroundColor: C.error + '14' },
    continueBtnTxt:{ fontSize: 15, fontWeight: '600', color: C.error },
    cancelBtn:     { alignItems: 'center', paddingVertical: 10 },
    cancelBtnTxt:  { fontSize: 14, color: C.textMuted },
    altList:       { gap: 10, marginBottom: 20 },
    altCard:       { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: C.surface, borderRadius: 16, padding: 14, borderWidth: 1, borderColor: C.border },
    ...
    altDesc:       { fontSize: 12, color: C.textMuted },
    divider:       { height: 1, backgroundColor: C.border, marginVertical: 20 },
    confirmField:  { marginBottom: 16 },
    confirmLabel:  { fontSize: 13, color: C.textSec, marginBottom: 8, lineHeight: 20 },
    inputWrap:     { flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface, borderRadius: 14, paddingHorizontal: 14, height: 50, borderWidth: 1, borderColor: C.border },
    input:         { flex: 1, color: C.text, fontSize: 14 },
    errorBox:      { flexDirection: 'row', gap: 8, backgroundColor: C.error + '18', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: C.error + '40', marginBottom: 16, alignItems: 'flex-start' },
    errorTxt:      { flex: 1, fontSize: 13, color: C.error, lineHeight: 18 },
    deleteBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: C.error, borderRadius: 16, paddingVertical: 16, marginBottom: 12 },
    deleteBtnTxt:  { fontSize: 15, fontWeight: '700', color: '#fff' },
    deletedIcon:   { width: 100, height: 100, borderRadius: 30, backgroundColor: C.success + '20', borderWidth: 1, borderColor: C.success + '40', alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
    deletedTitle:  { fontSize: 24, fontWeight: '800', color: C.text, textAlign: 'center', marginBottom: 12 },
    deletedSub:    { fontSize: 14, color: C.textSec, textAlign: 'center', lineHeight: 22, marginBottom: 20 },
    deletedFine:   { fontSize: 12, color: C.textMuted, textAlign: 'center', lineHeight: 18 },
  });
  ...
  if (step === 'deleting') {
    return (
      <View style={[s.root, { alignItems: 'center', justifyContent: 'center', gap: 20 }]}>
        ...
        <Text style={[s.deletedTitle, { color: C.error }]}>Deleting your account...</Text>
        <Text style={s.deletedSub}>Please wait. Do not close the app.</Text>
      </View>
    );
  }

  const steps = ['warning', 'alternatives', 'confirm'];
    ...
    <View style={s.root}>
          ...
          return (
            <View key={i} style={s.stepItem}>
              <View style={[s.stepCircle, done && s.stepCircleDone]}>
                <Text style={[s.stepNum, done && s.stepNumDone]}>{i + 1}</Text>
              </View>
              {i < 2 && <View style={[s.stepLine, i < idx && s.stepLineDone]} />}
            </View>
          );
        })}
      </View>
      ...
      >
        {step === 'warning' && (
          <>
            <View style={s.warningIcon}>
              <Ionicons name="warning" size={44} color={C.error} />
            </View>
              ...
              {WHAT_GETS_DELETED.map((item, i) => (
                <View key={i} style={[s.deleteRow, i === WHAT_GETS_DELETED.length - 1 && { borderBottomWidth: 0 }]}>
                  <View style={s.deleteIconWrap}>
                    <Ionicons name={item.icon as any} size={18} color={C.error} />
                  </View>
            ...
            <TouchableOpacity style={s.continueBtn} onPress={() => setStep('alternatives')} activeOpacity={0.85}>
              ...
              <Ionicons name="arrow-forward" size={18} color={C.error} />
            </TouchableOpacity>
            ...
            <View style={s.altList}>
              {ALTERNATIVES.map((alt, i) => (
                <TouchableOpacity key={i} style={s.altCard} onPress={() => handleAlternative(alt.action)} activeOpacity={0.8}>
                  <View style={[s.altIcon, { backgroundColor: alt.color + '20' }]}>
        ...
        {step === 'confirm' && (
          <>
            <View style={s.warningIcon}>
              <Ionicons name="skull-outline" size={44} color={C.error} />
            </View>
            ...
            <View style={s.confirmField}>
              <Text style={s.confirmLabel}>
                Type <Text style={{ color: C.error, fontWeight: '700' }}>{CONFIRM_PHRASE}</Text> to confirm:
              </Text>
              <View style={[s.inputWrap, confirmMatches && { borderColor: C.error }]}>
                <TextInput
                  style={s.input} value={confirmTxt} onChangeText={setConfirmTxt}
                  placeholder={CONFIRM_PHRASE} placeholderTextColor={C.textMuted}
                  autoCapitalize="characters" autoCorrect={false}
                />
                {confirmMatches && <Ionicons name="checkmark-circle" size={18} color={C.error} />}
              </View>
            </View>
            <View style={s.confirmField}>
              <Text style={s.confirmLabel}>Enter your password:</Text>
              <View style={s.inputWrap}>
                <Ionicons name="lock-closed-outline" size={17} color={C.textMuted} style={{ marginRight: 8 }} />
                <TextInput
                  style={s.input} value={password}
                  onChangeText={v => { setPassword(v); setError(''); }}
                  placeholder="Your current password" placeholderTextColor={C.textMuted}
