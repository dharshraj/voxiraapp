...
export default function LiveInterviewScreen({ navigation, route }: any) {
  ...
  // Collect the user's typed/verbal answers to pass to FeedbackScreen → ScoreBreakdown
  ...
  const submitAnswer = async () => {
    ...
    const answer   = currentAnswer.trim() || `(Answered verbally in ${formatTime(timer)})`;

    // Record this answer regardless of evaluation outcome
  ...
  const goToFeedback = () => {
    ...
    const evalsSnapshot   = [...evaluations];
    const avgScore = scoresSnapshot.length
      ? Math.round(scoresSnapshot.reduce((a, b) => a + b, 0) / scoresSnapshot.length)
      : 70;
  ...
  useEffect(() => {
    if (phase === 'done') setTimeout(goToFeedback, 1000);
  }, [phase]);
